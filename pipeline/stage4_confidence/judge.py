...
JUDGE_HINT = (
    ...
    "注意区分伪造与输入错配：若外部动作和返回在字幕中明确出现，但选中图属于另一题、另一镜头或讲解包装帧，"
    "只把该瑕疵写入 input_quality_alignment（及 notes），"
    "不得因为当前图片看不到该结果而重复命中 fabricated_observation，"
    "也不得把选图质量记入 hard_gates；"
    ...
    "带讲解水印/字幕条/包装帧或与轨迹所依原图不一致时只在本维扣分，"
    "不得另记选图质量硬门槛或软审查码；"
