...
- 合格 `create`/`new_operation` 写入本 task 提案队列（`stage3_tool_proposals.json`），**不得**自动改官方 `canonical_tool_catalog_v2.json`；提案定义必须包含小写下划线 name、description、executor、usage、至少一个带完整 v2 `input_schema`（fields 含 type、`requirement_level`、description、`acquisition_hint`）的 operation；运行时外层参数固定为 `operation/purpose/inputs`
- 程序门失败或置信度不足：本条用临时 slug 继续，mapping 标记 `temporary`，阶段4 `notes` 必须汇报；临时名不单独压 `decision`
