...
2. **阶段1.5（审核切分）**：结合字幕 + 稀疏抽帧做语义审核（禁止词表特判）：
   ...
   - **切分粒度**：一个 task = 一次独立定位题 = 一条最终 `final_answer` 链。同题多图（共同支撑同一地点、或后图精化前图）合并为 **一个** task 且 `multi_target_images=true`；仅不同目标/不同最终地点才拆多 task。
   - **关键帧 = 被定位对象本身**；截帧后经 VLM 验收，地图/街景应用/钉点/讲解遮罩等 `teaching_ui` 不得入 `image_paths`。
